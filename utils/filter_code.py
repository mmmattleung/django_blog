...
class FilterList(object):
    ...
    def __iter__(self):
        ...
        if self.option.name in self.param_dict:
            ...
            yield mark_safe("<a href='{0}' class='btn btn-default btn-rounded'>全部</a>".format(url))
        else:
            url = "{0}?{1}".format(self.path_info,self.param_dict.urlencode())
            yield mark_safe("<a class='active btn btn-default btn-rounded' href='{0}'>全部</a>".format(url))
        ...
        for row in self.queryset:
            ...
            if active:
                tpl = "<a class='active btn btn-default btn-rounded' href='{0}'>{1}</a>".format(url,text)
            else:
                tpl = "<a href='{0}' class='btn btn-default btn-rounded'>{1}</a>".format(url, text)
            yield mark_safe(tpl)
